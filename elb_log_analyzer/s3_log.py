...
class S3LogFetcher:
    ...
    def download_log_and_extract(self, s3_file_key:str, local_file_path:str):
        # download file
        local_file_path_dir = dirname(local_file_path)
        makedirs(local_file_path_dir, exist_ok=True)
        self.s3_client.download_file(self._bucket_name, s3_file_key, local_file_path)
    ...
    def s3_fetch_logs(self, store_location:str, start_time:datetime, end_time:datetime):
        '''
        ...
        for obj in response['Contents']:
            ...
            if start_time <= modified_time  <= end_time:
                ...
                # download file
                self.download_log_and_extract(key, local_file_path)
